...
```bash
wget http://localhost:3000/swagger.json -O swagger-api.json
```
...
```powershell
Invoke-WebRequest -Uri http://localhost:3000/swagger.json -OutFile swagger.json
```
...
https.get('http://localhost:3000/swagger.json', (response) => {
  response.pipe(file);
  file.on('finish', () => {
    file.close();
...
response = requests.get('http://localhost:3000/swagger.json')
...
# دانلود
curl -o test-swagger.json http://localhost:3000/swagger.json
...
# دانلود
curl -s -o swagger.json http://localhost:3000/swagger.json
...
- **Download Endpoint**: http://localhost:3000/swagger.json
...
3. Health check کنید: `curl http://localhost:3000/api/health`
