...
1. باز کنید: http://91.99.13.17/api-docs
...
```bash
wget http://91.99.13.17/swagger.json -O swagger-api.json
```
...
```powershell
Invoke-WebRequest -Uri http://91.99.13.17/swagger.json -OutFile swagger.json
```
...
https.get('http://91.99.13.17/swagger.json', (response) => {
  response.pipe(file);
  file.on('finish', () => {
    file.close();
...
response = requests.get('http://91.99.13.17/swagger.json')
...
# دانلود
curl -o test-swagger.json http://91.99.13.17/swagger.json
...
# چک کردن سرور
if ! curl -s http://91.99.13.17/api/health > /dev/null; then
...
# دانلود
curl -s -o swagger.json http://91.99.13.17/swagger.json
...
- **Download Endpoint**: http://91.99.13.17/swagger.json
...
نه:
- ❌ http://91.99.13.17/api/swagger.json
- ❌ http://91.99.13.17/api-docs/swagger.json
...
3. Health check کنید: `curl http://91.99.13.17/api/health`
