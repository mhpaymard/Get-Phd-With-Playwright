...
Go to: `http://91.99.13.17:3000/api-docs`
...
async function quickSearch(keywords) {
  // 1. Create session
  const sessionResp = await fetch('http://91.99.13.17:3000/api/session', {
    method: 'POST',
  ...
  // 2. Search
  const searchResp = await fetch('http://91.99.13.17:3000/api/search', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      userId: 'quick-user',
...
def quick_search(keywords):
    BASE = 'http://91.99.13.17:3000/api'
