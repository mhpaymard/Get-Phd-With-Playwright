...
[![Swagger](https://img.shields.io/badge/API-Swagger-brightgreen.svg)](http://91.99.13.17/api-docs)
...
# باز کردن مرورگر: http://91.99.13.17/api-docs
```
...
**مشاهده در Swagger**: http://91.99.13.17/api-docs
...
docker run -p 3001 findaphd-api
```
...
async function searchPhD(keywords) {
  ...
  // جستجو
  const result = await fetch('http://91.99.13.17/api/search', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      userId: 'user-123',
...
def search_phd(keywords):
    # ایجاد session
    session = requests.post('http://91.99.13.17/api/session', 
                           json={'userId': 'user-123'}).json()
    
    # جستجو
    result = requests.post('http://91.99.13.17/api/search', json={
        'userId': 'user-123',
        'sessionId': session['data']['sessionId'],
        'keywords': keywords,
        'filters': {'discipline': '10M7g0'}
...
### نتیجه‌ای برنمی‌گرده
- چک کنید سرور در حال اجرا باشد: `curl http://91.99.13.17/api/health`
- Swagger UI رو چک کنید: http://91.99.13.17/api-docs
