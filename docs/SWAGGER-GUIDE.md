...
│                                                         │
│  Servers:                                               │
│  ▼ http://91.99.13.17:3000/api  [Development Server]     │
│                                                         │
├─────────────────────────────────────────────────────────┤
...
**Request URL:**
```
http://91.99.13.17:3000/api/health
```
...
curl "http://91.99.13.17:3000/api/search/${SEARCH_ID}?sessionId=${SESSION_ID}"

# 4. صفحه بعدی
curl -X POST "http://91.99.13.17:3000/api/search/${SEARCH_ID}/continue" \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "'$SESSION_ID'",
    "page": 2
...
# 5. مشاهده تاریخچه
curl "http://91.99.13.17:3000/api/search/history/${SESSION_ID}"

# 6. حذف Session
curl -X DELETE "http://91.99.13.17:3000/api/session/${SESSION_ID}"
```
...
const config = new Configuration({
  basePath: 'http://91.99.13.17:3000/api'
});
...
```javascript
// .env.development
API_BASE_URL=http://91.99.13.17:3000/api
...
async function checkAvailability() {
  const health = await fetch('http://91.99.13.17:3000/api/health/ready');
...
# مرورگر: http://91.99.13.17:3000/api-docs
