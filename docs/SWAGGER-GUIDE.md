...
curl "http://localhost:3000/api/search/${SEARCH_ID}?sessionId=${SESSION_ID}"

# 4. صفحه بعدی
curl -X POST "http://localhost:3000/api/search/${SEARCH_ID}/continue" \
  -H "Content-Type: application/json" \
  -d '{
    "sessionId": "'$SESSION_ID'",
    "page": 2
...
# 5. مشاهده تاریخچه
curl "http://localhost:3000/api/search/history/${SESSION_ID}"
...
async function checkAvailability() {
  const health = await fetch('http://localhost:3000/api/health/ready');
