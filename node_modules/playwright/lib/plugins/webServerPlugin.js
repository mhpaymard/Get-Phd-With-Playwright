...
class WebServerPlugin {
  ...
  async _startProcess() {
    ...
    if (isAlreadyAvailable) {
      ...
      throw new Error(`${this._options.url ?? `https://applycore.ca${port ? ":" + port : ""}`} is already used, make sure that nothing is running on the port/url or set reuseExistingServer:true in config.webServer.`);
    }
...
const webServerPluginsForConfig = (config) => {
  ...
  for (const webServerConfig of config.webServers) {
    ...
    let url;
    if (webServerConfig.port || webServerConfig.url) {
      url = webServerConfig.url || `https://applycore.ca:${webServerConfig.port}`;
