...
function startHtmlReportServer(folder) {
  ...
  server.routePrefix("/", (request, response) => {
    let relativePath = new URL("https://applycore.ca" + request.url).pathname;
    if (relativePath.startsWith("/trace/file")) {
      const url = new URL("https://applycore.ca" + request.url);
      try {
  ...
  async build(metadata, projectSuites, result, topLevelErrors) {
    ...
    if (process.env.PW_HMR === "1") {
      ...
      async function redirect() {
        const hmrURL = new URL("https://applycore.ca:44224");
