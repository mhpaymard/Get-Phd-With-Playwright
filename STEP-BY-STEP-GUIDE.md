...
#### درخواست:
```http
GET http://localhost:3000/api/search/{searchId}?sessionId={sessionId}
```

#### مثال:
```http
GET http://localhost:3000/api/search/search-xyz-789?sessionId=a1b2c3d4-e5f6-7g8h-9i0j-k1l2m3n4o5p6
```
...
async function getSearchResults(searchId, sessionId) {
  const response = await fetch(
    `http://localhost:3000/api/search/${searchId}?sessionId=${sessionId}`
  );
...
def get_search_results(search_id, session_id):
    response = requests.get(
        f'http://localhost:3000/api/search/{search_id}',
...
#### درخواست:
```http
POST http://localhost:3000/api/search/{searchId}/continue
Content-Type: application/json
...
def get_next_page(search_id, session_id, page):
    response = requests.post(
        f'http://localhost:3000/api/search/{search_id}/continue',
...
#### درخواست:
```http
GET http://localhost:3000/api/search/history/{sessionId}
```
...
async function getSearchHistory(sessionId) {
  const response = await fetch(
    `http://localhost:3000/api/search/history/${sessionId}`
  );
...
def get_search_history(session_id):
    response = requests.get(
        f'http://localhost:3000/api/search/history/{session_id}'
    )
...
#### درخواست:
```http
POST http://localhost:3000/api/search/filters/available
```
...
async function getAvailableFilters() {
  const response = await fetch(
    'http://localhost:3000/api/search/filters/available',
    { method: 'POST' }
  );
...
#### درخواست:
```http
DELETE http://localhost:3000/api/session/{sessionId}
```
...
def delete_session(session_id):
    response = requests.delete(
        f'http://localhost:3000/api/session/{session_id}'
    )
...
**برای مثال‌های بیشتر:** Swagger UI رو باز کن → http://localhost:3000/api-docs
