...
#### درخواست:
```http
GET http://91.99.13.17/api/health
```
...
**JavaScript:**
```javascript
const response = await fetch('http://91.99.13.17/api/health');
...
response = requests.get('http://91.99.13.17/api/health')
...
#### درخواست:
```http
POST http://91.99.13.17/api/session
...
#### درخواست:
```http
GET http://91.99.13.17/api/search/{searchId}?sessionId={sessionId}
```

#### مثال:
```http
GET http://91.99.13.17/api/search/search-xyz-789?sessionId=a1b2c3d4-e5f6-7g8h-9i0j-k1l2m3n4o5p6
```
...
async function getSearchResults(searchId, sessionId) {
  const response = await fetch(
    `http://91.99.13.17/api/search/${searchId}?sessionId=${sessionId}`
  );
...
def get_search_results(search_id, session_id):
    response = requests.get(
        f'http://91.99.13.17/api/search/{search_id}',
...
#### درخواست:
```http
POST http://91.99.13.17/api/search/{searchId}/continue
Content-Type: application/json
...
def get_next_page(search_id, session_id, page):
    response = requests.post(
        f'http://91.99.13.17/api/search/{search_id}/continue',
...
#### درخواست:
```http
GET http://91.99.13.17/api/search/history/{sessionId}
```
...
async function getSearchHistory(sessionId) {
  const response = await fetch(
    `http://91.99.13.17/api/search/history/${sessionId}`
  );
...
def get_search_history(session_id):
    response = requests.get(
        f'http://91.99.13.17/api/search/history/{session_id}'
    )
...
#### درخواست:
```http
POST http://91.99.13.17/api/search/filters/available
```
...
async function getAvailableFilters() {
  const response = await fetch(
    'http://91.99.13.17/api/search/filters/available',
    { method: 'POST' }
  );
...
#### درخواست:
```http
DELETE http://91.99.13.17/api/session/{sessionId}
```
...
async function deleteSession(sessionId) {
  const response = await fetch(
    `http://91.99.13.17/api/session/${sessionId}`,
    { method: 'DELETE' }
  );
...
def delete_session(session_id):
    response = requests.delete(
        f'http://91.99.13.17/api/session/{session_id}'
    )
...
class PhDSearchClient {
  constructor(baseUrl = 'http://91.99.13.17/api') {
...
class PhDSearchClient:
    def __init__(self, base_url='http://91.99.13.17/api'):
...
```javascript
// 1. Session بساز
const session = await fetch('http://91.99.13.17/api/session', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
...
// 2. جستجو کن
const search = await fetch('http://91.99.13.17/api/search', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    userId: 'my-user',
...
**برای مثال‌های بیشتر:** Swagger UI رو باز کن → http://91.99.13.17/api-docs
