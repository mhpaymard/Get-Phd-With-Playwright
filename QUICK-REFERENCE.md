...
```bash
curl -X POST http://91.99.13.17/api/search/search-456/continue \
  -H "Content-Type: application/json" \
  -d '{
...
if (search.data.totalPages > 1) {
  const page2 = await fetch(
    `http://91.99.13.17/api/search/${search.searchId}/continue`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
if search['data']['totalPages'] > 1:
    page2 = requests.post(
        f"http://91.99.13.17/api/search/{search['searchId']}/continue",
